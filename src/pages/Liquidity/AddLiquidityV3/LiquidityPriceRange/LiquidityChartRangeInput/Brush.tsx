...
import { brushHandle, OffScreenHandle } from './svg'
...
export const Brush = ({
  ...
  const brushed = useCallback(
    (event: D3BrushEvent<unknown>) => {
      ...
      if (type === 'end' && !compare(brushExtent, scaled, xScale) && mode) {
      ...
      <>
        <defs>
          ...
          <clipPath id={`${id}-brush-clip`}>
            <rect x="0" y="0" width={innerWidth} height={innerHeight} rx="10" ry="10" />
          </clipPath>

          <clipPath id="clip-rounded-corners">
            {/* Adjust x, y, width, height to match your chart's position and dimensions */}
            <rect x="0" y="0" width={innerWidth - 3} height={innerHeight} rx="10" ry="10" />
          </clipPath>
        </defs>
        ...
        {localBrushExtent && (
          <>
            ...
            {westHandleInView ? (
              <g
                transform={`translate(${Math.max(0, xScale(localBrushExtent[0]))}, -2), scale(${
                  flipWestHandle ? '-1' : '1'
                }, 1)`}
              >
                {brushHandle(innerHeight * 1.05)}
              </g>
            ) : null}

            {/* east handle */}
            {eastHandleInView ? (
              <g transform={`translate(${xScale(localBrushExtent[1])}, -2), scale(${flipEastHandle ? '-1' : '1'}, 1)`}>
                {brushHandle(innerHeight * 1.05)}
              </g>
            ) : null}
