...
describe('hooks', () => {
  describe('#queryParametersToSwapState', () => {
    test('FUSE to DAI', () => {
      const queryParameters = queryParametersToSwapState(
        parse(
    ...
    test('output FUSE only', () => {
      expect(
        queryParametersToSwapState(
          parse('?outputCurrency=fuse&exactAmount=20.5', { parseArrays: false, ignoreQueryPrefix: true })
        )
      ).toEqual({
    ...
    // We do not have swaps to recipients implemented
